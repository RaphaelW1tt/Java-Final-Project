...
public class SimplePanel extends JPanel{
	...
	public void paintComponent(Graphics g) {
		...
		for(Circle c:content.getCircles()){
			...
			Ellipse2D.Double shape = new Ellipse2D.Double(c.getPoint().getX()-(c.getDiameter()/2) ,c.getPoint().getY()-(c.getDiameter()/2), c.getDiameter(), c.getDiameter() );
		    g2d.draw(shape);
		...
		for(Rectangle r: content.getRectangles()){
			...
			Rectangle2D.Double shape = new Rectangle2D.Double(r.getPoint().getX(), r.getPoint().getY(), r.getWidth(), r.getHeight());		  
			g2d.draw(shape);
